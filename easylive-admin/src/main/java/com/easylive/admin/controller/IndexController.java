...
public class IndexController extends ABaseController {
    ...
    public ResponseVO getActualTimeStatisticsInfo() {
        ...
        //查询前一天用户总数，替换类型为粉丝的数量
        UserInfoQuery userInfoQuery=new UserInfoQuery();
        userInfoQuery.setJoinTime(preDate);
        Integer userCount = userInfoService.findCountByParam(userInfoQuery);
        preDayData = preDayData.stream().filter(item -> !item.getDataType().equals(StatisticsTypeEnum.FANS.getType())).collect(Collectors.toList());
        StatisticsInfo statisticsInfo=new StatisticsInfo();
        statisticsInfo.setStatisticsCount(userCount);
        statisticsInfo.setStatisticsDate(preDate);
        statisticsInfo.setDataType(StatisticsTypeEnum.FANS.getType());
        preDayData.add(statisticsInfo);
